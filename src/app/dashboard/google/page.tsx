...
const generateGoogleMockData = () => {
  ...
  for (let i = 30; i >= 0; i--) {
    ...
    data.push({
      ...
      cpl: parseFloat((Math.random() * 8 + 4).toFixed(2)),
      impressions: Math.floor(Math.random() * 6000 + 2500),
      clicks: Math.floor(Math.random() * 600 + 100),
      ctr: parseFloat((Math.random() * 2.5 + 1).toFixed(2)),
      spend: parseFloat((Math.random() * 180 + 40).toFixed(2))
    })
  }
  return data
}
...
export default function GoogleDashboardPage() {
  ...
  return (
    <div className="space-y-12">
      <section>
        <h2 className="text-xl font-semibold mb-4">Visão Geral - Google Ads</h2>
